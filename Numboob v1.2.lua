...
function gameanswer(msg)
 if msg.fromGroup == "0" then --私聊执行模块
    ...
    local answer = getnum(msg.fromMsg)
    local max = getUserConf(msg.fromQQ,"boobmax",300)
    local min = getUserConf(msg.fromQQ,"boobmin",1)
    local key = getUserConf(msg.fromQQ,"boobkey")
    if answer == key then
